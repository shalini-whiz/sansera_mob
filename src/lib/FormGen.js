          ...
          <View
            ...
            {groupItem.map((item, index) => {
              return (
                <View
                  key={index}
                  style={{backgroundColor: 'white', flex: 1, margin: 5}}>
                  <View
                    style={
                      this.props.labelDataInRow
                        ? styles.formDataItemRow
                        : styles.formItem
                    }>
                    {item.displayName && item.displayName.length ? (
                      <Text style={styles.formLabel}>{item.displayName}</Text>
                    ) : (
                      false
                    )}
                    {item.select ? (
                      <Picker
                        selectedValue={
                          item.titleCase
                            ? util.capitalizeWord(item.value)
                            : item.value
                        }
                        ...
                        {item.options &&
                          item.options.map((pickerItem, pickerIndex) => {
                            let labelV =
                              item.keyName && pickerItem[item.keyName]
                                ? pickerItem[item.keyName]
                                : pickerItem.key
                                ? pickerItem.key
                                : pickerItem;
                            let label =
                              item.valueName && pickerItem[item.valueName]
                                ? pickerItem[item.valueName]
                                : pickerItem.value
                                ? pickerItem.value
                                : pickerItem;
              ...
              <View
                ...
                {item.select ? (
                  <Picker
                    ...
                    style={[
                      this.props.labelDataInRow
                        ? styles.pickerItemInRow
                        : styles.pickerItem,
                      {
                        backgroundColor:
                          this.props.editMode === false ? 'red' : '#ECF0FA',
                      },
                    ]}
                    itemStyle={{}}
                    dropdownIconColor={appTheme.colors.cardTitle}>
                  ...
                  <TextInput
                    ...
                    style={[
                      this.props.labelDataInRow
                        ? styles.textInputInRow
                        : styles.TextInput,
                      {
                        backgroundColor:
                          this.props.editMode === false ? 'white' : '#ECF0FA',
                        paddingLeft: '3%', // by rakshith
                      },
                    ]}
                    placeholder={item.placeholder}
                    placeholderTextColor="#fafafa"
