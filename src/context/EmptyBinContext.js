...
const EmptyBinContextProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    AsyncStorage.getItem("filledBinCount")
      .then(data => {
        if (data) setUnReadFilledBin(data);
      })
    AsyncStorage.getItem("taskCount")
      .then(data => {
        if (data) setUnReadTask(data);
      })
    AsyncStorage.getItem("lowBattery")
      .then(data => {
  ...
  return (
    <EmptyBinContext.Provider value={{
      unReadEmptyBin, setUnReadEmptyBinData,
      unReadFilledBin, setUnReadFilledBinData, unReadTask, resetTaskData, appProcess, 
      setAppProcessData, lowBattery, setLowBattery
    }}>
      {children}
