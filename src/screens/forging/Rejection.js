...
export const Rejection = React.memo(props => {
  ...
  const handleChange = name => async (value, index) => {
    if (name === 'reason') {
      ...
      if (level2) {
        let level2Obj = curRejection[curKey].find(item => {
          return (
            Object.keys(item).length && Object.keys(item)[0] === rejSubType
          );
        });
        let nestedObj = level2Obj[rejSubType].find(item => {
          return Object.keys(item).length && Object.keys(item)[0] === value;
        });
        if (nestedObj) {
          setCurRejCount(nestedObj[value]);
        }
      } else {
        let nestedObj = curRejection[curKey].find(item => {
          return Object.keys(item).length && Object.keys(item)[0] === value;
        });
        if (nestedObj) {
          setCurRejCount(nestedObj[value]);
    ...
    if (name === 'subType') {
      ...
      let nestedObj = curObj.find(
        item => Object.keys(item).length && Object.keys(item)[0] === value,
      );

      let subTypeReasonList = await nestedObj[value].reduce(
        (keys, obj) =>
      ...
      let curReason = nestedObj[value].find(item => {
        return (
          Object.keys(item).length &&
          Object.keys(item)[0] === subTypeReasonList[0]
        );
      });

      if (curReason) setCurRejCount(curReason[subTypeReasonList[0]]);
    }
  };
  const handleRejection = async item => {
    ...
    setCurRejMenu(key);
    let level1Keys = await item[key].reduce(
      (keys, obj) =>
        keys.concat(Object.keys(obj).filter(key => keys.indexOf(key) === -1)),
      [],
    );
    ...
    if (Array.isArray(item[key][0][level1Keys[0]])) {
      ...
      let nestedObj = item[key].find(item => {
        return (
          Object.keys(item).length && Object.keys(item)[0] === presetSubType
        );
      });
      if (nestedObj) {
        let subTypeReasonList = await nestedObj[presetSubType].reduce(
          (keys, obj) =>
            keys.concat(
              Object.keys(obj).filter(key => keys.indexOf(key) === -1),
            ),
        ...
        let curReason = nestedObj[presetSubType].find(item => {
          return (
            Object.keys(item).length &&
            Object.keys(item)[0] === subTypeReasonList[0]
          );
      ...
      let nestedObj = item[key].find(item => {
        return (
          Object.keys(item).length && Object.keys(item)[0] === level1Keys[0]
        );
      });
      if (nestedObj) {
  ...
  const updateRejections = async () => {
    ...
    ApiService.getAPIRes(apiData, 'POST', 'rejection').then(apiRes => {
      ...
      if (apiRes && apiRes.status) {
        setRejCount(0);
        Alert.alert('Rejection Count Updated');
        props.updateProcess();
        loadData();
      } else if (apiRes && apiRes.response.message)
        setApiError(apiRes.response.message);
    ...
    <ScrollView
      ...
      <View style={styles.mainContainer}>
        <View style={{flexDirection: 'row'}}>
                  ...
                  <Text
                    ...
                    style={[
                      AppStyles.sideMenu,
                      {
                        ...
                        padding: 10,
                        backgroundColor:
                          rejectionMenu[0] == Object.keys(curRejection)[0]
          ...
          <View
            style={{flex: 6, margin: 5, padding: 5, backgroundColor: 'white'}}>
            {curRejection && level1.length && !level2 ? (
              <View style={{flexDirection: 'column'}}>
                ...
                <View
                  ...
                  <Text style={[AppStyles.filterLabel, {flex: 1}]}>
                    Rejection Count <Text style={{color: 'red'}}>* </Text> (
                    {curRejCount})
                  </Text>
                  <TextInput
            ...
            {rejError.length ? (
              <Text style={AppStyles.error}>{rejError}</Text>
            ) : (
              false
            )}
            <View
