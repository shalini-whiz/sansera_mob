...
export const ConsumptionData = React.memo(props => {
  ...
  const loadProcess = async () => {
    ...
    if (curStage.toLowerCase() === stageType.shearing) {
      ...
      ApiService.getAPIRes(apiData, 'POST', 'process_consumption').then(
        apiRes => {
          setApiStatus(false);
    ...
    return (
      <View style={[styles.tableData, {flexDirection: 'row'}]} key={index}>
        ...
        {tableSchema.map((tabItem, tabIndex) => {
          ...
          if (tabItem.type === 'date') {
            value = dateUtil.fromToDateFormat(
              value,
              'DD/MM/YYYY, hh:mm:ss A',
              'DD MMM YYYY',
            );
            //value = dateUtil.toDateFormat(value, "DD/MM/yyyy")
            //hh:mm A
          }
          if (tabItem.type === 'time') {
            //value = dateUtil.toTimeFormat(value,"h:mm:ss a")
            value = dateUtil.fromToDateFormat(
              value,
              'DD/MM/YYYY, hh:mm:ss A',
              'hh:mm A',
          ...
          return tabItem.key != 'slNo' ? (
            <Text style={[styles.tableDataCell, {}]} key={tabIndex}>
              {value}
            </Text>
          ) : (
            false
  ...
  return (
    <View style={[styles.container, {}]}>
      ...
      <View
        ...
        <TouchableOpacity
          ...
          <FontAwesome
            name="calendar"
            size={25}
            color={appTheme.colors.cardTitle}></FontAwesome>
          <Text style={[AppStyles.subtitleWithBold, {marginLeft: 10}]}>
            {dateUtil.toDateFormat(date, 'DD MMM yyyy')}
          </Text>
        </TouchableOpacity>
      </View>
      {show ? (
              ...
              return (
                <View style={styles.tableHeader}>
                  {tableSchema.map((item, index) => {
                    ...
                    return (
                      <Text
                        style={[styles.tableHeaderCell, customStyle]}
                        key={index}>
                        {item.displayName}
                      </Text>
                    );
      ...
      {apiMsg && apiMsg.length ? (
        <Text style={[AppStyles.subtitle, {textAlign: 'center'}]} type="info">
          {apiMsg}
        </Text>
      ) : (
        false
...
const styles = StyleSheet.create({
  ...
  tableDataCell: {
    ...
    borderColor: 'grey',
    color: 'black',
    borderBottomWidth: 0.5, //by Rakshith
  },
});