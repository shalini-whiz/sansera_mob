...
export const Rejection = React.memo(props => {
    ...
    ApiService.getAPIRes(apiData, 'POST', 'rejection').then(async apiRes => {
      ...
      if (apiRes) {
        if (
          ...
          let menuKeys = await rejections[0][curKey].reduce(
            (keys, obj) =>
              keys.concat(
          ...
          if (rejReason === '') {
            ...
            let nestedObj = rejections[0][curKey].find(item => {
              return (
                Object.keys(item).length &&
                Object.keys(item)[0] === initalReason
              );
            });
            if (nestedObj) {
  ...
  const handleChange = name => (value, index) => {
    if (name === 'reason') {
      ...
      if (nestedObj) setCurRejCount(nestedObj[value]);
    }
  };
