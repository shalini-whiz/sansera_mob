...
export const BinTask = React.memo(props => {
  ...
  const loadData = async () => {
    ...
    if (stage && stage.toLowerCase() === stageType.shotblasting) {
      let inputStageList = await appProcess.process.reduce(async function (
        accumulator,
        item,
      ) {
  ...
  const updateRequest = () => {
    ...
    if (dialogType === 'fulfilled') {
      ...
      invokeApi = true;
    } else if (dialogType === 'cancel') apiData.task_state = taskState.CANCEL;
    else if (dialogType === 'delivered') apiData.task_state = taskState.DELIVER;
    closeDialog(); // by Rakshith
    ApiService.getAPIRes(apiData, 'POST', 'task').then(apiRes => {
      if (apiRes && apiRes.status) {
        let msg = '';
        if (dialogType === 'fulfilled') {
          ...
          AsyncStorage.getItem('deviceDet').then(async devices => {
            ...
            let device = devicesDet.find(
              device => device.element_num === binNo,
            );
            if (device) {
        ...
        Alert.alert(msg);
        loadData();
      } else if (apiRes.response.message) setApiError(apiRes.response.message);
    });
  };
  const errOKAction = () => {
    setApiError('');
