...
export const ProcessFifo = React.memo(props => {
  ...
  const switchStage = (e, stage) => {
    ...
    if (stage.sub_stage && stage.sub_stage.length) {
      let subIndex = proStageData.findIndex(
        item => item.stage_name === stage.sub_stage,
      );
  ...
  const updateBin = () => {
    ...
    let invokeApi = false;
    if (delSubBin.length) {
      let elementIndex = subStage.fifo.findIndex(
        item => item.element_num === delSubBin[0],
      );
    ...
    if (delBin.length) {
      let elementIndex = stage.fifo.findIndex(
        item => item.element_num === delBin[0],
      );
      apiData.op = 'remove_element';
      apiData.process_name = props.processDet.process_name;
    ...
    if (invokeApi) {
      ApiService.getAPIRes(apiData, 'POST', 'fifo').then(apiRes => {
        ...
        } else setApiError('Could not update. try again!');
      });
    }
  };
          ...
          <View
                    ...
                    <Text
                      ...
                      style={[
                        styles.tableHeader,
                        {
                          ...
                          borderWidth: 0.5,
                          color:
                            stage.stage_name === item.stage_name
                          ...
                          style={[
                            {
                              ...
                              fontSize: 18,
                              color:
                                delBin.indexOf(fifoItem.element_num) > -1
                                  ? 'red'
                                  : 'black',
                            },
                          ]}>
            ...
            {delBin.length ? (
              <Text style={{color: 'red', fontSize: 14, padding: 10}}>
                Only one bin can be removed at a time
              </Text>
            ) : (
              false
            )}
            {delBin.length || delSubBin.length ? (
              <View
                ...
                <TouchableOpacity
                  ...
                  <Text style={styles.sucButtonText}>
                    {isConfirm ? 'CONFIRM' : 'REMOVE'}
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[
                    styles.canButtonContainer,
                    {
                      flex: 1,
                      padding: 5,
                      paddingLeft: 10,
                ...
                return (
                  <View style={{flexDirection: 'column'}} key={index}>
                    ...
                    <Text
                      ...
                      {item.last_popped_element &&
                      item.last_popped_element.length
                        ? item.last_popped_element
                        : ' '}
                    </Text>
                  </View>
