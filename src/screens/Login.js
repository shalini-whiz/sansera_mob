...
let loginSchema = [
  {
    ...
    icon: (
      <SvgCss
        xml={EmailIcon(appTheme.colors.cardTitle)}
        width={20}
        height={20}
        style={{margin: 5}}
      />
  ...
  },
  {
    ...
    icon: (
      <SvgCss
        xml={PasswordIcon(appTheme.colors.cardTitle)}
        width={20}
        height={20}
        style={{margin: 5}}
      />
...
export default function Login() {
  ...
  const handleChange = name => value => {
    ...
    if (index != -1) {
      ...
      let updatedLoginData = [
        ...loginFormData.slice(0, index),
        updatedItem,
        ...loginFormData.slice(index + 1),
      ];
  ...
  const handleSubmit = async () => {
    ...
    if (!isError) {
      ...
      setApiStatus(false);
      if (!apiRes) setApiError('Please load again!');
      if (apiRes && apiRes.status) {
        if (apiRes.response.role === 'QA')
          apiRes.response.roleName = 'Quality Operator';
        AsyncStorage.setItem(
          'userInfo',
          JSON.stringify(apiRes.response.message),
        );
        AsyncStorage.setItem('token', apiRes.response.message.accessToken);
        let defaultLowBattery = [];
        AsyncStorage.setItem('lowBattery', JSON.stringify(defaultLowBattery));
        ...
        let topicPaylaod = {
          op: 'get_device',
          unit_num: apiRes.response.message.unit_number,
        };
        ...
        if (topicRes.status) {
          ...
          let devices = [];
          let deviceDet = [];
          let switchPressedRacks = topicRes.response.message.reduce(function (
            acc,
            obj,
          ) {
  ...
  return (
    <>
      {isLoggedIn ? (
        <Home user={user} binTopics={binTopics} />
      ) : (
        <View style={styles.loginContainer}>
              ...
              <View
                ...
                <View
                  ...
                  <View
                    ...
                    />
                    {item.secureTextEntry != undefined &&
                    item.value &&
                    item.value.length ? (
                      <Icon
                        ...
                        onPress={e => setVisibility(item.key)}
                        style={{
          ...
          <View
            ...
            <TouchableOpacity
              style={[styles.loginBtn, {flexDirection: 'row'}]}
